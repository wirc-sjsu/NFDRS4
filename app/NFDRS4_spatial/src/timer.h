...
private:
    ...
    bool outputFile;
public:
    Timer(bool outputFile = false) : start(std::chrono::high_resolution_clock::now()), outputFile(outputFile) {}
    ...
    long stop(){
        ...
        std::cout << "Duration: " <<  duration << reinterpret_cast<const char*>(" us (") << durationMS << " ms)" << std::endl;

        if(outputFile) {
            std::ofstream myfile;
            myfile.open ("statistics.txt", std::ios_base::app);
            myfile << duration << "\n";
            myfile.close();
        }
        return duration;
    ...
    long stop(std::ostream& stream){
        ...
        stream << "Duration: " <<  duration << reinterpret_cast<const char*>(" us (") << durationMS << " ms)" << std::endl;
        return duration;
    }
